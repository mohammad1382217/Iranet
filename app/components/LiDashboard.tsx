...
const LiDashboard: React.FC<LiDashboardProps> = ({item}) => {
  ...
  return (
    <NavLink
      ...
      className="flex flex-row gap-2.5 h-10 transition ease-in-out delay-100 duration-100 items-center justify-start px-4 py-2 rounded-lg text-sm font-medium"
      key={item.id}
      to={item.redirect}
      style={({ isActive }) =>
        isActive
